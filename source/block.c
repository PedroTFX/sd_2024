...
struct block_t *block_duplicate(struct block_t *b){
    ...
    struct block_t* block_dup = malloc(sizeof(struct block_t));
    if(!block_dup){
        return NULL;
    }

    block_dup->datasize = b->datasize;
    block_dup->data = malloc(b->datasize);
    if(block_dup->data){
        free(block_dup);            //might need to be block_destroy
        return NULL;
    }

    memcpy(block_dup->data, b->data, b->datasize);
    return block_dup;
}
...
int block_replace(struct block_t *b, int new_size, void *new_data){
    ...
    return (b->datasize == new_size && b->data == new_data) ? 0 : -1;
}
